...
class JobQueue {
  constructor() {
    ...
    videos.forEach((video) => {
      Object.entries(video.resizes).forEach((entry) => {
        const [width, height] = entry[0].split("x");
        if (entry[1].processing) {
          this.enqueue({
            type: "resize",
            video,
            width,
  ...
  /**
   *
   * @param {object} job - contain the `type` and all the processing attribute we need
   */
  enqueue(job) {
    this.jobs.push(job);
    this.executeNext();
  ...
  async execute(job) {
    ...
    if (job.type === "resize") {
      ...
      try {
        ...
        DB.save();
        if (updateVideo.userId)
          util.invalidateCache(updateVideo.userId.toString());
        console.log(
